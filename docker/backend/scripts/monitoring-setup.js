...
exports.MonitoringSetup = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const logger_1 = require("../src/services/logger");
class MonitoringSetup {
    static MONITORING_DIR = './monitoring';
    static PROMETHEUS_DIR = './deploy/prometheus';
    static GRAFANA_DIR = './deploy/grafana';
    static ALERTMANAGER_DIR = './deploy/alertmanager';
    static async setup() {
        logger_1.LoggerService.info('📊 Setting up monitoring infrastructure...');
        try {
            ...
            logger_1.LoggerService.info('✅ Monitoring setup completed successfully');
        }
        catch (error) {
            logger_1.LoggerService.error('❌ Monitoring setup failed:', error);
            throw error;
        }
    }
    static createDirectories() {
        ...
        for (const dir of dirs) {
            if (!fs.existsSync(dir)) {
                ...
                logger_1.LoggerService.info(`Created directory: ${dir}`);
            }
        }
    }
    static async setupPrometheus() {
        logger_1.LoggerService.info('Setting up Prometheus configuration...');
        ...
        logger_1.LoggerService.info(`✅ Prometheus configuration written to: ${configPath}`);
    }
    static async setupGrafana() {
        logger_1.LoggerService.info('Setting up Grafana dashboards...');
        ...
        logger_1.LoggerService.info(`✅ Backend performance dashboard created: ${dashboardPath}`);
        ...
        logger_1.LoggerService.info(`✅ Compliance dashboard created: ${compliancePath}`);
    }
    static async setupAlertManager() {
        logger_1.LoggerService.info('Setting up AlertManager configuration...');
        ...
        logger_1.LoggerService.info(`✅ AlertManager configuration written to: ${configPath}`);
    }
    static async setupCustomMetrics() {
        logger_1.LoggerService.info('Setting up custom metrics and alerts...');
        const alertsConfig = {
            groups: [
                {
        ...
        logger_1.LoggerService.info(`✅ Custom alerts configuration written to: ${alertsPath}`);
    }
    static async validate() {
        logger_1.LoggerService.info('🔍 Validating monitoring setup...');
        try {
            ...
            logger_1.LoggerService.info('✅ Monitoring configuration validation passed');
        }
        catch (error) {
            logger_1.LoggerService.error('❌ Monitoring configuration validation failed:', error);
            throw error;
        }
    }
}
