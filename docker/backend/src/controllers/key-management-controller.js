...
exports.KeyManagementController = void 0;
...
const MASTER_KEY = crypto.scryptSync(process.env.KEY_ENCRYPTION_SECRET || 'default-key-encryption-secret-change-in-production', 'salt', 32);
class KeyManagementController {
     ...
     * Log audit event
     */
    logAudit(keyId, action, performedBy, details, ipAddress) {
        const log = {
        ...
        try {
            ...
            }).catch(err => logger_1.LoggerService.warn('Failed to persist key audit log', { error: err.message }));
        }
        catch (err) {
    ...
    async createKey(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Creating new key', { tenantId, algorithm, keySize, purpose });
            // Generate key ID
            ...
            logger_1.LoggerService.info('Key created successfully', { keyId, tenantId });
            ...
            logger_1.LoggerService.error('Failed to create key', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async rotateKey(req, res) {
        try {
            ...
            const userId = user_id || req.user?.id || 'system';
            logger_1.LoggerService.info('Rotating key', { keyId, userId });
            // Get existing key
            ...
            if (existingKey.algorithm.startsWith('AES')) {
                ...
                newKeyMaterial = Buffer.from(privateKey);
            }
            else {
            ...
            logger_1.LoggerService.info('Key rotated successfully', { keyId, newVersion: existingKey.version });
            res.json({
                success: true,
                data: {
                    keyId,
            ...
            logger_1.LoggerService.error('Failed to rotate key', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async revokeKey(req, res) {
        try {
            ...
            const userId = user_id || req.user?.id || 'system';
            logger_1.LoggerService.info('Revoking key', { keyId, userId });
            // Get existing key
            const existingKey = keyStore.get(keyId);
            if (!existingKey) {
                res.status(404).json({
            ...
            logger_1.LoggerService.info('Key revoked successfully', { keyId });
            res.json({
                success: true,
                data: {
                    keyId,
            ...
            logger_1.LoggerService.error('Failed to revoke key', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async encryptData(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Encrypting data', { tenantId, keyId, dataLength: data?.length });
            let key;
            let usedKeyId = keyId;
            if (keyId) {
                // Use specified key
            ...
            logger_1.LoggerService.error('Failed to encrypt data', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async decryptData(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Decrypting data with HSM', { tenantId, dataLength: encryptedData?.length });
            ...
            logger_1.LoggerService.error('Failed to decrypt data', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async signData(req, res) {
        try {
            ...
            const { data, keyId } = req.body;
            logger_1.LoggerService.info('Signing data with HSM', { tenantId, keyId, dataLength: data?.length });
            ...
            logger_1.LoggerService.error('Failed to sign data', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async verifySignature(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Verifying signature with HSM', { tenantId, keyId });
            ...
            logger_1.LoggerService.error('Failed to verify signature', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async getKeyUsageStats(req, res) {
        try {
            ...
            logger_1.LoggerService.error('Failed to get key usage stats', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async getKeyAuditLogs(req, res) {
        try {
            ...
            logger_1.LoggerService.error('Failed to get key audit logs', { error: error.message });
            res.status(500).json({
                success: false,
     ...
     */
    async checkKeyExpiration(req, res) {
        try {
            const tenantId = req.params.tenantId;
            if (!tenantId) {
                res.status(400).json({ message: 'tenantId is required', code: 'INVALID_REQUEST' });
            ...
            logger_1.LoggerService.error('Failed to check key expiration', { error: error.message });
            res.status(500).json({
                success: false,
    ...
    async getKeyHealth(req, res) {
        try {
            ...
            logger_1.LoggerService.error('Failed to get key health', { error: error.message });
            res.status(500).json({
                success: false,
...
exports.KeyManagementController = KeyManagementController;
//# sourceMappingURL=key-management-controller.js.map