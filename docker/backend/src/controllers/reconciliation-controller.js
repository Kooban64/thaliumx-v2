...
// In-memory stores
const reconciliationConfigs = new Map();
...
class ReconciliationController {
    ...
    async runDailyReconciliation(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Starting daily reconciliation', { tenantId });
            ...
            try {
                ...
                internalTransactions = transactions.map((t) => t.toJSON());
                // Calculate totals
            ...
            for (const source of tenantSources) {
                try {
                    ...
                    logger_1.LoggerService.info('Fetching from external source', { sourceId: source.id, sourceName: source.name });
                    // externalTransactions.push(...fetchedTransactions);
                }
                catch (fetchError) {
                    logger_1.LoggerService.warn('Failed to fetch from external source', {
                        sourceId: source.id,
            ...
            try {
                ...
                logger_1.LoggerService.warn('Could not persist reconciliation report', { error: dbError.message });
            }
            await event_streaming_1.EventStreamingService.emitSystemEvent('reconciliation.completed', 'ReconciliationService', discrepancies.length > 0 ? 'warn' : 'info', {
                reportId: report.id,
                tenantId,
            ...
            logger_1.LoggerService.info('Daily reconciliation completed', {
                reportId: report.id,
                tenantId,
            ...
            logger_1.LoggerService.error('Failed to run daily reconciliation', { error: error.message });
            res.status(500).json({
    ...
    async runReconciliation(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Starting reconciliation for date range', { tenantId, startDate: start, endDate: end });
            ...
            logger_1.LoggerService.error('Failed to run reconciliation', { error: error.message });
            res.status(500).json({
     ...
     * Fetch external transactions from configured sources
     */
    async fetchExternalTransactions(req, res) {
        try {
            const tenantId = req.params.tenantId;
            if (!tenantId) {
                res.status(400).json({ message: 'tenantId is required', code: 'INVALID_REQUEST' });
            ...
            const start = new Date(startDate);
            const end = new Date(endDate);
            logger_1.LoggerService.info('Fetching external transactions', { tenantId, startDate: start, endDate: end });
            ...
            for (const source of sourcesToFetch) {
                try {
                    ...
                    switch (source.type) {
                        case 'bank':
                            // Would call bank API
                            logger_1.LoggerService.info('Fetching from bank source', { sourceId: source.id });
                            // fetchedCount = await this.fetchFromBankAPI(source, start, end);
                            break;
                        case 'exchange':
                            // Would call exchange API
                            logger_1.LoggerService.info('Fetching from exchange source', { sourceId: source.id });
                            // fetchedCount = await this.fetchFromExchangeAPI(source, start, end);
                            break;
                        case 'payment_processor':
                            // Would call payment processor API
                            logger_1.LoggerService.info('Fetching from payment processor', { sourceId: source.id });
                            break;
                        case 'blockchain':
                            // Would scan blockchain
                            logger_1.LoggerService.info('Fetching from blockchain', { sourceId: source.id });
                            break;
                        default:
                            logger_1.LoggerService.info('Manual source - no automatic fetch', { sourceId: source.id });
                    }
                    ...
                    logger_1.LoggerService.error('Failed to fetch from source', { sourceId: source.id, error: errorMessage });
                    source.status = 'error';
                    source.updatedAt = new Date();
                    results.errors.push({
                        sourceId: source.id,
            ...
            logger_1.LoggerService.info('External transaction fetch completed', {
                tenantId,
            ...
            logger_1.LoggerService.error('Failed to fetch external transactions', { error: error.message });
            res.status(500).json({
     ...
     */
    async getReconciliationConfig(req, res) {
        try {
            const tenantId = req.params.tenantId;
            if (!tenantId) {
                res.status(400).json({ message: 'tenantId is required', code: 'INVALID_REQUEST' });
            ...
            logger_1.LoggerService.error('Failed to get reconciliation config', { error: error.message });
            res.status(500).json({
     ...
     */
    async updateReconciliationConfig(req, res) {
        try {
            const tenantId = req.params.tenantId;
            if (!tenantId) {
                res.status(400).json({ message: 'tenantId is required', code: 'INVALID_REQUEST' });
            ...
            const { enabled, schedule, autoReconcile, toleranceAmount, tolerancePercentage, notifyOnDiscrepancy, notificationEmails, retentionDays } = req.body;
            ...
            if (autoReconcile !== undefined)
                config.autoReconcile = autoReconcile;
            if (toleranceAmount !== undefined)
                config.toleranceAmount = toleranceAmount;
            if (tolerancePercentage !== undefined)
                config.tolerancePercentage = tolerancePercentage;
            if (notifyOnDiscrepancy !== undefined)
                config.notifyOnDiscrepancy = notifyOnDiscrepancy;
            ...
            logger_1.LoggerService.info('Reconciliation config updated', { tenantId });
            res.json({
                success: true,
            ...
            logger_1.LoggerService.error('Failed to update reconciliation config', { error: error.message });
            res.status(500).json({
     ...
     */
    async addExternalTransactionSource(req, res) {
        try {
            const tenantId = req.params.tenantId;
            if (!tenantId) {
                res.status(400).json({ message: 'tenantId is required', code: 'INVALID_REQUEST' });
            ...
            logger_1.LoggerService.info('External transaction source added', { sourceId: newSource.id, tenantId, type });
            res.status(201).json({
                success: true,
            ...
            logger_1.LoggerService.error('Failed to add external transaction source', { error: error.message });
            res.status(500).json({
     ...
     */
    async getExternalTransactionSources(req, res) {
        try {
            const tenantId = req.params.tenantId;
            if (!tenantId) {
                res.status(400).json({ message: 'tenantId is required', code: 'INVALID_REQUEST' });
            ...
            logger_1.LoggerService.error('Failed to get external transaction sources', { error: error.message });
            res.status(500).json({
     ...
     */
    async updateExternalTransactionSource(req, res) {
        try {
            ...
            source.updatedAt = new Date();
            logger_1.LoggerService.info('External transaction source updated', { sourceId, tenantId });
            res.json({
                success: true,
            ...
            logger_1.LoggerService.error('Failed to update external transaction source', { error: error.message });
            res.status(500).json({
     ...
     */
    async deleteExternalTransactionSource(req, res) {
        try {
            const { tenantId, sourceId } = req.params;
            if (!tenantId || !sourceId) {
                res.status(400).json({
            ...
            logger_1.LoggerService.info('External transaction source deleted', { sourceId, tenantId });
            res.json({
                success: true,
            ...
            logger_1.LoggerService.error('Failed to delete external transaction source', { error: error.message });
            res.status(500).json({
    ...
    async getReconciliationReports(req, res) {
        try {
            ...
            logger_1.LoggerService.error('Failed to get reconciliation reports', { error: error.message });
            res.status(500).json({
