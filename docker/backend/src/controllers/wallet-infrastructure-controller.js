...
exports.WalletInfrastructureController = void 0;
...
const walletSecuritySettings = new Map();
const walletBackups = new Map();
class WalletInfrastructureController {
    /**
     * Create wallet infrastructure for new user
     * Creates FIAT wallet and optionally crypto wallets with ledger accounts
     */
    async createUserWalletInfrastructure(req, res) {
        try {
            ...
            logger_1.LoggerService.info('Creating wallet infrastructure for user', { userId, tenantId, brokerId, currencies });
            ...
            for (const currency of currencies) {
                ...
                let ledgerAccountId;
                try {
                    const ledgerAccount = await blnkfinance_1.BlnkFinanceService.createAccount(walletId, `${userId} ${currency} Wallet`, blnkfinance_1.AccountType.ASSET, currency, tenantId);
                    ledgerAccountId = ledgerAccount.id;
                }
                catch (ledgerError) {
                    logger_1.LoggerService.warn('Could not create ledger account', { error: ledgerError.message });
                }
                const fiatWallet = {
                    id: walletId,
                    userId,
            ...
            try {
                ...
                logger_1.LoggerService.warn('Could not persist wallets to database', { error: dbError.message });
            }
            logger_1.LoggerService.info('Wallet infrastructure created', { userId, walletCount: createdWallets.length });
            res.status(201).json({
                success: true,
                data: {
                    userId,
            ...
            logger_1.LoggerService.error('Failed to create wallet infrastructure', { error: error.message });
            res.status(500).json({
                success: false,
        ...
        try {
            ...
            if (userWallets.length === 0) {
                try {
                    ...
                    userWallets = dbWallets.map((w) => w.toJSON());
                }
                catch (dbError) {
                    logger_1.LoggerService.warn('Could not fetch wallets from database', { error: dbError.message });
                }
            }
            res.status(200).json({
                success: true,
            ...
            logger_1.LoggerService.error('Failed to get user wallets', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
        ...
        try {
            ...
            if (!wallet) {
                try {
                    ...
                    if (dbWallet) {
                        wallet = dbWallet.toJSON();
                    }
                }
                catch (dbError) {
                    logger_1.LoggerService.warn('Could not fetch wallet from database', { error: dbError.message });
                }
            }
            if (!wallet) {
                res.status(404).json({
            ...
            logger_1.LoggerService.error('Failed to get wallet details', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
    ...
    async getWalletBalance(req, res) {
        try {
            ...
            if (wallet.ledgerAccountId) {
                try {
                    const ledgerBalance = await blnkfinance_1.BlnkFinanceService.getAccountBalance(wallet.ledgerAccountId);
                    if (ledgerBalance) {
                    ...
                    logger_1.LoggerService.warn('Could not fetch balance from ledger', { error: ledgerError.message });
                }
            }
            res.status(200).json({
                success: true,
            ...
            logger_1.LoggerService.error('Failed to get wallet balance', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
    ...
    async updateWalletStatus(req, res) {
        try {
            ...
            try {
                const WalletModel = database_1.DatabaseService.getModel('Wallet');
                await WalletModel.update({ status, updatedAt: new Date() }, { where: { id: walletId } });
            }
            catch (dbError) {
                logger_1.LoggerService.warn('Could not update wallet in database', { error: dbError.message });
            }
            logger_1.LoggerService.info('Wallet status updated', { walletId, previousStatus, newStatus: status, reason });
            res.status(200).json({
                success: true,
                data: {
                    walletId,
            ...
            logger_1.LoggerService.error('Failed to update wallet status', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
        ...
        try {
            ...
            if (transactions.length === 0) {
                try {
                    ...
                    transactions = dbTransactions.map((t) => t.toJSON());
                }
                catch (dbError) {
                    logger_1.LoggerService.warn('Could not fetch transactions from database', { error: dbError.message });
                }
            }
            // Apply filters
            if (type) {
            ...
            logger_1.LoggerService.error('Failed to get wallet transactions', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
        ...
        try {
            ...
            if (wallet.ledgerAccountId) {
                try {
                    ...
                    await blnkfinance_1.BlnkFinanceService.recordTransaction(description || `${type} transaction`, [
                        {
                    ...
                    ], wallet.tenantId, transaction.currency, blnkTxType, transaction.id, { walletId, description });
                    ...
                    logger_1.LoggerService.error('Ledger transaction failed', { error: ledgerError.message });
            ...
            try {
                ...
                logger_1.LoggerService.warn('Could not persist transaction to database', { error: dbError.message });
            }
            logger_1.LoggerService.info('Wallet transaction created', {
            ...
            logger_1.LoggerService.error('Failed to create wallet transaction', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
     ...
     */
    async getExternalWalletProviders(_req, res) {
        try {
            const providers = [
                {
                    id: 'metamask',
            ...
            logger_1.LoggerService.error('Failed to get external wallet providers', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
     ...
     */
    async connectExternalWallet(req, res) {
        try {
            const { userId, tenantId, brokerId } = req.params;
            const { providerId, walletAddress, network } = req.body;
            logger_1.LoggerService.info('Connecting external wallet', { userId, providerId, walletAddress, network });
            const wallet = {
            ...
            logger_1.LoggerService.error('Failed to connect external wallet', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
     ...
     */
    async getWalletSecuritySettings(req, res) {
        try {
            const { walletId } = req.params;
            if (!walletId) {
                res.status(400).json({
            ...
            logger_1.LoggerService.error('Failed to get wallet security settings', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
     ...
     */
    async updateWalletSecuritySettings(req, res) {
        try {
            const { walletId } = req.params;
            const { twoFactorEnabled, whitelistedAddresses, withdrawalLimits, requireApprovalAbove } = req.body;
            if (!walletId) {
                res.status(400).json({
                    success: false,
                    error: 'INVALID_REQUEST',
            ...
            if (whitelistedAddresses !== undefined)
                settings.whitelistedAddresses = whitelistedAddresses;
            if (withdrawalLimits !== undefined)
                settings.withdrawalLimits = { ...settings.withdrawalLimits, ...withdrawalLimits };
            if (requireApprovalAbove !== undefined)
                settings.requireApprovalAbove = requireApprovalAbove;
            settings.lastUpdated = new Date();
            // Store settings
            walletSecuritySettings.set(walletId, settings);
            logger_1.LoggerService.info('Wallet security settings updated', { walletId });
            res.status(200).json({
                success: true,
                data: {
                    walletId,
            ...
            logger_1.LoggerService.error('Failed to update wallet security settings', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
    ...
    async getWalletBackup(req, res) {
        try {
            ...
            logger_1.LoggerService.error('Failed to get wallet backup', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
     ...
     */
    async createWalletBackup(req, res) {
        try {
            const { walletId } = req.params;
            ...
            const key = encryptionKey
                ? crypto_1.default.scryptSync(encryptionKey, 'salt', 32)
            ...
            // Calculate checksum
            const checksum = crypto_1.default.createHash('sha256').update(encryptedData).digest('hex');
            ...
            logger_1.LoggerService.info('Wallet backup created', { walletId, backupId: backup.id });
            res.status(201).json({
                success: true,
                data: {
                    walletId,
            ...
            logger_1.LoggerService.error('Failed to create wallet backup', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
     ...
     */
    async restoreWalletFromBackup(req, res) {
        try {
            ...
            // Verify checksum
            const currentChecksum = crypto_1.default.createHash('sha256').update(backup.encryptedData).digest('hex');
            ...
            try {
                ...
                logger_1.LoggerService.info('Wallet backup restored', { walletId, backupId });
                res.status(200).json({
                    success: true,
                    data: {
                        walletId,
            ...
            logger_1.LoggerService.error('Failed to restore wallet backup', { error: error.message });
            res.status(500).json({
                success: false,
                error: error.message || 'INTERNAL_ERROR',
...
exports.WalletInfrastructureController = WalletInfrastructureController;
//# sourceMappingURL=wallet-infrastructure-controller.js.map