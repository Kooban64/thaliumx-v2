...
const keycloak_1 = require("../services/keycloak");
 ...
 */
router.post('/brokers', error_handler_1.authenticateToken, error_handler_1.validateRequest, async (req, res) => {
    try {
        ...
        const result = await keycloak_1.KeycloakService.createBrokerRealm(defaultConfig);
        logger_1.LoggerService.info(`Broker realm created: ${brokerConfig.brokerId}`, {
        ...
        logger_1.LoggerService.error('Create broker realm failed:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Internal server error
 */
router.get('/brokers', error_handler_1.authenticateToken, async (req, res) => {
    try {
        const brokers = keycloak_1.KeycloakService.getBrokerRealms();
        res.json({
            success: true,
        ...
        logger_1.LoggerService.error('Get broker realms failed:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Internal server error
 */
router.post('/users', error_handler_1.authenticateToken, error_handler_1.validateRequest, async (req, res) => {
    try {
        ...
        const user = await keycloak_1.KeycloakService.createUser(realmName, {
            ...userData,
        ...
        logger_1.LoggerService.info(`User created in realm ${realmName}: ${userData.username}`, {
            userId: user.id,
            realm: realmName,
        ...
        logger_1.LoggerService.error('Create user failed:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Internal server error
 */
router.get('/users/:realmName/:userId', error_handler_1.authenticateToken, async (req, res) => {
    try {
        ...
        const user = await keycloak_1.KeycloakService.getUser(realmName, userId);
        if (!user) {
            res.status(404).json({
                success: false,
 ...
 *         description: Internal server error
 */
router.put('/users/:realmName/:userId', error_handler_1.authenticateToken, error_handler_1.validateRequest, async (req, res) => {
    try {
        const { realmName, userId } = req.params;
        ...
        await keycloak_1.KeycloakService.updateUser(realmName, userId, updates);
 ...
 *         description: Internal server error
 */
router.post('/migrate', error_handler_1.authenticateToken, error_handler_1.validateRequest, async (req, res) => {
    try {
        ...
        const result = await keycloak_1.KeycloakService.migrateUser({
        ...
        logger_1.LoggerService.info('User migration completed', {
            userId: migrationRequest.userId,
 ...
 *         description: Internal server error
 */
router.post('/validate', error_handler_1.authenticateToken, error_handler_1.validateRequest, async (req, res) => {
    try {
        ...
        const userInfo = await keycloak_1.KeycloakService.validateToken(realmName, token);
        res.json({
            success: true,
            valid: true,
            userInfo
 ...
 *         description: Internal server error
 */
router.get('/roles/:realmName', error_handler_1.authenticateToken, async (req, res) => {
    try {
        ...
        const roles = await keycloak_1.KeycloakService.getRealmRoles(realmName);
        res.json({
            success: true,
 ...
 *         description: Internal server error
 */
router.post('/users/:realmName/:userId/roles', error_handler_1.authenticateToken, error_handler_1.validateRequest, async (req, res) => {
    try {
        const { realmName, userId } = req.params;
        ...
        await keycloak_1.KeycloakService.assignRolesToUser(realmName, userId, roles);
...
router.get('/health', async (req, res) => {
    try {
        const isHealthy = keycloak_1.KeycloakService.isHealthy();
        const brokers = keycloak_1.KeycloakService.getBrokerRealms();
        res.json({
            ...
            details: {
                initialized: keycloak_1.KeycloakService.isHealthy(),
        ...
        logger_1.LoggerService.error('Keycloak health check failed:', error);
        res.status(500).json({
            status: 'unhealthy',
            service: 'keycloak',
            timestamp: new Date().toISOString(),
