...
exports.initializeNativeCEX = void 0;
const express_1 = require("express");
const native_cex_1 = require("../services/native-cex");
const error_handler_1 = require("../middleware/error-handler");
const logger_1 = require("../services/logger");
const database_1 = require("../services/database");
const router = (0, express_1.Router)();
let nativeCEXService;
// Initialize service
const initializeNativeCEX = async () => {
    try {
        nativeCEXService = new native_cex_1.NativeCEXService(database_1.DatabaseService.getSequelize());
        await nativeCEXService.initialize();
        logger_1.LoggerService.info('Native CEX routes initialized');
    }
    catch (error) {
        logger_1.LoggerService.error('Failed to initialize Native CEX', { error });
        throw error;
    }
};
exports.initializeNativeCEX = initializeNativeCEX;
// ==================== TRADING ENGINES ====================
// Get trading engines status
router.get('/engines', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.get('/pairs', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.get('/pairs/thal', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
// ==================== ORDER MANAGEMENT ====================
// Place order
router.post('/orders', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const order = await nativeCEXService.placeOrder(userId, tenantId, brokerId, {
            tradingPairId,
            side,
            type,
            quantity,
...
// Get user's orders
router.get('/orders/user/:userId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
// Cancel order
router.delete('/orders/:orderId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const { orderId } = req.params;
        ...
        logger_1.LoggerService.error('Failed to cancel order', { error, orderId: req.params.orderId });
        next(error);
    }
});
// ==================== MARKET DATA ====================
// Get market data for symbol
router.get('/market-data/:symbol', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        logger_1.LoggerService.error('Failed to get market data', { error, symbol: req.params.symbol });
        next(error);
    }
});
// Get market data for all THAL pairs
router.get('/market-data/thal/all', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const thalPairs = nativeCEXService.getTHALTradingPairs();
        const marketDataPromises = thalPairs.map(pair => nativeCEXService.getMarketData(pair.symbol).catch(error => {
            logger_1.LoggerService.warn('Failed to get market data for THAL pair', { error, symbol: pair.symbol });
            return null;
        }));
...
router.get('/thal/business-model', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.get('/thal/incentives/:userId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            res.status(400).json({
...
router.post('/thal/credit-rewards', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        logger_1.LoggerService.error('Failed to credit THAL rewards', { error, body: req.body });
        next(error);
    }
});
// ==================== DASHBOARD & ANALYTICS ====================
...
router.get('/analytics/platform', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
        const engines = nativeCEXService.getTradingEngines();
        const pairs = nativeCEXService.getTradingPairs();
        const thalPairs = nativeCEXService.getTHALTradingPairs();
