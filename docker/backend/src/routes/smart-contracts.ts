 ...
 */
router.post('/deploy', authenticateToken, validateRequest, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    LoggerService.info(`Contract deployed: ${contractName}`, {
 ...
 *         description: Internal server error
 */
router.get('/token/:contractAddress/info', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const tokenInfo = await SmartContractService.getTokenInfo(contractAddress);
 ...
 *         description: Internal server error
 */
router.post('/token/:contractAddress/transfer', authenticateToken, validateRequest, async (req: Request, res: Response): Promise<void> => {
  try {
    const { contractAddress } = req.params;
    ...
    const result = await SmartContractService.executeTransaction(
      contractAddress,
      SmartContractService['CONTRACT_ABIS'].THAL_TOKEN,
      'transfer',
      [to, amount]
    );
 ...
 *         description: Internal server error
 */
router.get('/presale/:contractAddress/info', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { contractAddress } = req.params;
    ...
    const presaleInfo = await SmartContractService.getPresaleInfo(contractAddress);
 ...
 *         description: Internal server error
 */
router.post('/presale/:contractAddress/buy', authenticateToken, validateRequest, async (req: Request, res: Response): Promise<void> => {
  try {
    const { contractAddress } = req.params;
    ...
    const result = await SmartContractService.executeTransaction(
      contractAddress,
      SmartContractService['CONTRACT_ABIS'].PRESALE,
      'buyTokens',
      [],
      value
    );
...
router.get('/staking/:contractAddress/info', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const stakingInfo = await SmartContractService.getStakingInfo(
      contractAddress,
 ...
 *         description: Internal server error
 */
router.post('/staking/:contractAddress/stake', authenticateToken, validateRequest, async (req: Request, res: Response): Promise<void> => {
  try {
    const { contractAddress } = req.params;
    ...
    const result = await SmartContractService.executeTransaction(
      contractAddress,
      SmartContractService['CONTRACT_ABIS'].STAKING,
      'stake',
      [amount]
    );
    ...
    LoggerService.error('Staking failed:', error);
    res.status(500).json({
      success: false,
 ...
 *         description: Internal server error
 */
router.post('/:contractAddress/call', authenticateToken, validateRequest, async (req: Request, res: Response): Promise<void> => {
  try {
    const { contractAddress } = req.params;
    ...
    const result = await SmartContractService.callContract(
      contractAddress,
      abi,
      method,
      args
 ...
 *         description: Internal server error
 */
router.post('/:contractAddress/execute', authenticateToken, validateRequest, async (req: Request, res: Response): Promise<void> => {
  try {
    const { contractAddress } = req.params;
 ...
 *         description: Internal server error
 */
router.get('/network', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const networkId = await SmartContractService.getNetworkId();
    ...
    LoggerService.error('Get network info failed:', error);
    res.status(500).json({
      success: false,
...
router.get('/health', async (req: Request, res: Response): Promise<void> => {
  try {
    const isHealthy = SmartContractService.isHealthy();
    const networkId = await SmartContractService.getNetworkId();
    ...
    res.json({
      ...
      details: {
        initialized: SmartContractService.isHealthy(),
        networkId: networkId,
    ...
    LoggerService.error('Smart contract health check failed:', error);
    res.status(500).json({
      status: 'unhealthy',
      service: 'smart-contracts',
      timestamp: new Date().toISOString(),
