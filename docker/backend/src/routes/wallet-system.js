...
exports.initializeWalletSystem = void 0;
const express_1 = require("express");
const wallet_system_1 = require("../services/wallet-system");
const nedbank_1 = require("../services/nedbank");
const error_handler_1 = require("../middleware/error-handler");
const logger_1 = require("../services/logger");
const database_1 = require("../services/database");
const router = (0, express_1.Router)();
let walletSystemService;
// Initialize service
const initializeWalletSystem = async () => {
    try {
        walletSystemService = new wallet_system_1.WalletSystemService(database_1.DatabaseService.getSequelize());
        await walletSystemService.initialize();
        await nedbank_1.NedbankService.initialize();
        logger_1.LoggerService.info('Wallet System routes initialized');
    }
    catch (error) {
        logger_1.LoggerService.error('Failed to initialize Wallet System', { error });
        throw error;
    }
};
exports.initializeWalletSystem = initializeWalletSystem;
// ==================== WALLET MANAGEMENT ====================
// Create wallet infrastructure for new user
router.post('/infrastructure', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const wallets = await walletSystemService.createUserWalletInfrastructure(userId, tenantId, brokerId, userInfo);
        res.json({
            success: true,
            data: {
                userId,
        ...
        logger_1.LoggerService.error('Failed to create wallet infrastructure', { error, body: req.body });
        next(error);
    }
});
// Get user's wallets
router.get('/user/:userId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
// Get specific wallet
router.get('/wallet/:walletId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.post('/reference/generate', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const reference = await walletSystemService.generateUniqueReference(userId, tenantId, brokerId, referenceType, currency, expectedAmount);
        res.json({
            success: true,
            data: {
        ...
        logger_1.LoggerService.error('Failed to generate unique reference', { error, body: req.body });
        next(error);
    }
});
// Get persistent FIAT deposit reference (alphanumeric)
router.get('/reference/persistent/:currency', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.get('/reference/:reference', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.get('/convert/quote', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const userId = req.user?.id;
        const tenantId = req.tenantId || 'default-tenant';
        const brokerId = req.brokerId || 'default-broker';
...
// Confirm conversion
router.post('/convert/confirm', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const userId = req.user?.id;
        const tenantId = req.tenantId || 'default-tenant';
        const brokerId = req.brokerId || 'default-broker';
...
router.post('/deposit/fiat', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const result = await walletSystemService.processFiatDeposit(reference, actualAmount, bankTransaction);
        res.json({
        ...
        logger_1.LoggerService.error('Failed to process FIAT deposit', { error, body: req.body });
        next(error);
    }
});
// ==================== POOL ACCOUNT MANAGEMENT ====================
// Create pool account
router.post('/pool-accounts', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
        ...
        const poolAccount = await walletSystemService.createPoolAccount(brokerId, accountType, bankDetails);
        res.json({
            success: true,
            data: {
                poolAccount: {
        ...
        logger_1.LoggerService.error('Failed to create pool account', { error, body: req.body });
        next(error);
    }
});
// ==================== BANK WITHDRAWALS (Nedbank) ====================
router.post('/withdrawals/bank', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const userId = req.user?.id;
        ...
        const payout = await nedbank_1.NedbankService.initiatePayout({
            brokerId,
            poolAccountNumber,
            beneficiary,
            amount,
...
router.get('/deposits/scrape', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
        const { brokerId, poolAccountNumber, fromDate, toDate } = req.query;
        const records = await nedbank_1.NedbankService.scrapeDeposits({
...
router.post('/fiat-admin/unallocated', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
...
router.get('/fiat-admin/unallocated', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
        const { brokerId, currency, status } = req.query;
...
router.post('/fiat-admin/allocation/propose', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
...
router.post('/fiat-admin/allocation/approve', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
...
router.post('/fiat-admin/allocation/reject', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
...
router.post('/fiat-admin/allocation/execute', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
...
router.post('/deposits/apply', error_handler_1.authenticateToken, (0, error_handler_1.requireRole)(['admin', 'super_admin']), async (req, res, next) => {
    try {
...
router.get('/pool-accounts/:brokerId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
// ==================== CEX INTEGRATION ====================
// Place CEX order
router.post('/cex/orders', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const order = await walletSystemService.placeCEXOrder(userId, tenantId, brokerId, {
            tradingPair,
            side,
            type,
            quantity,
        ...
        logger_1.LoggerService.error('Failed to place CEX order', { error, body: req.body });
        next(error);
    }
});
// Get user's CEX orders
router.get('/cex/orders/:userId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            res.status(400).json({
...
router.get('/thal/rewards/:userId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            res.status(400).json({
...
// ==================== WALLET RECOVERY ====================
// Recover hot wallet
router.post('/recovery/hot-wallet', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        ...
        const result = await walletSystemService.recoverHotWallet(userId, mfaCode, recoveryMethod);
        res.json({
            success: result.success,
            data: {
                userId,
        ...
        logger_1.LoggerService.error('Failed to recover hot wallet', { error, body: req.body });
        next(error);
    }
});
// ==================== DASHBOARD & ANALYTICS ====================
// Get wallet dashboard
router.get('/dashboard/:userId', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            res.status(400).json({
...
router.get('/statements', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
...
router.get('/tax-report', error_handler_1.authenticateToken, async (req, res, next) => {
    try {
        const userId = req.user?.id;
        const { from, to, method = 'fifo', baseCurrency = 'ZAR' } = req.query;
        if (!userId) {
            res.status(401).json({ success: false, error: 'Unauthorized' });
            return;
        }
        const csv = await walletSystemService.generateTaxReportCSV({ userId, from: from ? String(from) : undefined, to: to ? String(to) : undefined, method: method, baseCurrency: String(baseCurrency) });
        res.setHeader('Content-Type', 'text/csv');
