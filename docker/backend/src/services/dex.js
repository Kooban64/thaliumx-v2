...
exports.DEXService = exports.PoolType = exports.DEXProvider = exports.SwapStatus = exports.ChainId = exports.DEXProtocol = void 0;
...
(function (DEXProtocol) {
    ...
    DEXProtocol["TRADERJOE"] = "traderjoe";
    DEXProtocol["SPOOKYSWAP"] = "spookyswap";
    DEXProtocol["SPIRITSWAP"] = "spiritswap";
    DEXProtocol["JOE"] = "joe";
    DEXProtocol["PANGOLIN"] = "pangolin";
    DEXProtocol["ZEROX"] = "0x";
...
(function (DEXProvider) {
    ...
    DEXProvider["SUSHISWAP"] = "sushiswap";
    DEXProvider["PANCAKESWAP"] = "pancakeswap";
    DEXProvider["THALIUM_DEX"] = "thalium_dex";
})(DEXProvider || (exports.DEXProvider = DEXProvider = {}));
var PoolType;
(function (PoolType) {
    PoolType["CONSTANT_PRODUCT"] = "constant_product";
...
class DEXService {
    ...
    static swaps = new Map();
    static priceFeeds = new Map();
    static liquidityPositions = new Map();
    static dexConfigs = new Map();
    static providers = new Map();
    static tokenCache = new Map();
    static poolCache = new Map();
    ...
    static async initialize() {
        try {
            logger_1.LoggerService.info('Initializing DEX Service...');
            ...
            await event_streaming_1.EventStreamingService.emitSystemEvent('dex.initialized', 'DEXService', 'info', {
    ...
    static initializeDEXConfigs() {
        ...
        logger_1.LoggerService.info('DEX configurations initialized', {
    ...
    static initializeProviders() {
        ...
        chains.forEach(({ id, url }) => {
            this.providers.set(id, new ethers_1.JsonRpcProvider(url));
        });
        logger_1.LoggerService.info('Blockchain providers initialized', {
    ...
    static async getTokenInfo(chainId, tokenAddress) {
        ...
        if (!provider) {
            throw (0, utils_1.createError)(`Provider not available for chain ${chainId}`, 400, 'PROVIDER_NOT_AVAILABLE');
        }
        try {
            const tokenContract = new ethers_1.ethers.Contract(tokenAddress, ['function symbol() view returns (string)', 'function decimals() view returns (uint8)', 'function name() view returns (string)'], provider);
            if (!tokenContract || !tokenContract.symbol || !tokenContract.decimals || !tokenContract.name) {
                throw (0, utils_1.createError)('Token contract methods not available', 400, 'CONTRACT_METHODS_UNAVAILABLE');
            }
            ...
            if (!symbol || decimals === undefined || !name) {
                throw (0, utils_1.createError)('Failed to fetch token information from contract', 400, 'TOKEN_INFO_FETCH_FAILED');
            }
            const tokenInfo = {
            ...
            logger_1.LoggerService.error('Failed to get token info', { error, chainId, tokenAddress });
            throw (0, utils_1.createError)('Failed to get token info', 500, 'TOKEN_INFO_ERROR');
        }
    }
    /**
    ...
    static getSupportedChains() {
        return Object.values(ChainId).filter(v => typeof v === 'number');
    }
    /**
     * Get best quote across all DEXs
     */
    static async getBestQuote(tokenIn, tokenOut, amountIn, slippage = this.DEX_CONFIG.defaultSlippage, options) {
        try {
            ...
            for (const dex of dexesToQuery) {
                ...
                for (let attempt = 1; attempt <= 2; attempt++) {
                    try {
                        ...
                        lastError = error;
                        if (attempt === 2) {
                            logger_1.LoggerService.warn(`Failed to get quote from ${dex} after retries`, { error });
                        }
                    }
                }
            }
            if (quotes.length === 0) {
                throw (0, utils_1.createError)('No quotes available from any DEX', 404, 'NO_QUOTES_AVAILABLE');
            }
            ...
            logger_1.LoggerService.info('Best quote found', {
                dex: bestQuote.dex,
     ...
     */
    static async executeSwap(userId, tenantId, brokerId, tokenIn, tokenOut, amountIn, slippage, deadline, route) {
        try {
            ...
            if (slippage > this.DEX_CONFIG.maxSlippage) {
                throw (0, utils_1.createError)(`Slippage ${slippage}% exceeds maximum allowed ${this.DEX_CONFIG.maxSlippage}%`, 400, 'SLIPPAGE_TOO_HIGH');
            }
            // Validate deadline
            if (deadline < Date.now() / 1000) {
                throw (0, utils_1.createError)('Deadline has already passed', 400, 'DEADLINE_PASSED');
            }
            ...
            logger_1.LoggerService.info('Swap executed successfully', {
                swapId: swap.id,
                userId: swap.userId,
            ...
            // Emit audit event
            await event_streaming_1.EventStreamingService.emitAuditEvent('swap.executed', 'dex', swapId, {
                userId,
                tenantId,
                brokerId,
                tokenIn,
     ...
     */
    static async addLiquidity(userId, poolId, token0Amount, token1Amount, slippage = this.DEX_CONFIG.defaultSlippage) {
        try {
            ...
            if (!pool) {
                throw (0, utils_1.createError)('Pool not found', 404, 'POOL_NOT_FOUND');
            }
            if (!pool.isActive) {
                throw (0, utils_1.createError)('Pool is not active', 400, 'POOL_NOT_ACTIVE');
            }
            ...
            logger_1.LoggerService.info('Liquidity added successfully', {
            ...
            // Emit audit event
            await event_streaming_1.EventStreamingService.emitAuditEvent('liquidity.added', 'dex', positionId, {
                userId,
                poolId,
                token0Amount,
                token1Amount,
     ...
     */
    static async removeLiquidity(userId, positionId, lpTokenAmount, slippage = this.DEX_CONFIG.defaultSlippage) {
        try {
            ...
            if (!position) {
                throw (0, utils_1.createError)('Liquidity position not found', 404, 'POSITION_NOT_FOUND');
            }
            if (position.userId !== userId) {
                throw (0, utils_1.createError)('Unauthorized to remove this liquidity position', 403, 'UNAUTHORIZED');
            }
            if (!position.isActive) {
                throw (0, utils_1.createError)('Liquidity position is not active', 400, 'POSITION_NOT_ACTIVE');
            }
            // Process liquidity removal
            await this.processLiquidityRemoval(position, lpTokenAmount);
            logger_1.LoggerService.info('Liquidity removed successfully', {
                positionId: position.id,
                userId: position.userId,
                lpTokenAmount
            });
            // Emit audit event
            await event_streaming_1.EventStreamingService.emitAuditEvent('liquidity.removed', 'dex', positionId, {
                userId,
                positionId,
                lpTokenAmount,
    ...
    static async getSwapById(swapId) {
        try {
            ...
            logger_1.LoggerService.error('Get swap by ID failed:', error);
            throw error;
        }
    }
    /**
     ...
     */
    static async getUserLiquidityPositions(userId) {
        try {
     ...
     * Get liquidity pools
     */
    static async getLiquidityPools(options) {
        try {
            ...
            if (options?.token0) {
                pools = pools.filter(pool => pool.token0.toLowerCase() === options.token0.toLowerCase() ||
                    pool.token1.toLowerCase() === options.token0.toLowerCase());
            }
            if (options?.token1) {
                pools = pools.filter(pool => pool.token0.toLowerCase() === options.token1.toLowerCase() ||
    ...
    static async getPoolById(poolId) {
        try {
            ...
            logger_1.LoggerService.error('Get pool by ID failed:', error);
            throw error;
        }
    }
    /**
     * Get price feeds
     */
    static async getPriceFeeds(tokens) {
        try {
    ...
    static async getDEXStats() {
        try {
            ...
            const byDEX = this.DEX_CONFIG.supportedDEXs.map(dex => {
            ...
            const byToken = this.DEX_CONFIG.supportedTokens.map(token => {
            ...
            const recentSwaps = swaps.filter(swap => new Date(swap.createdAt).getTime() > Date.now() - 24 * 60 * 60 * 1000).length;
    ...
    static async close() {
        try {
            logger_1.LoggerService.info('Closing DEX Service...');
            this.isInitialized = false;
            this.pools.clear();
            this.swaps.clear();
            this.priceFeeds.clear();
            this.liquidityPositions.clear();
            logger_1.LoggerService.info('✅ DEX Service closed');
        }
        catch (error) {
            logger_1.LoggerService.error('Error closing DEX Service:', error);
            throw error;
        }
    }
    // =============================================================================
    ...
    static async validateConfiguration() {
        try {
            ...
            logger_1.LoggerService.info('DEX configuration validated successfully');
        }
        catch (error) {
    ...
    static async loadExistingData() {
        try {
            ...
            logger_1.LoggerService.info('Existing DEX data loaded from database');
        }
        catch (error) {
    ...
    static async initializePriceFeeds() {
        try {
            ...
            logger_1.LoggerService.info('Price feeds initialized');
        }
        catch (error) {
            logger_1.LoggerService.error('Initialize price feeds failed:', error);
            throw error;
        }
    }
    static async initializeLiquidityPools() {
        try {
            ...
            logger_1.LoggerService.info('Liquidity pools initialized');
        }
        catch (error) {
            logger_1.LoggerService.error('Initialize liquidity pools failed:', error);
            throw error;
        }
    }
    static async startPriceFeedUpdater() {
        try {
            setInterval(async () => {
                try {
                    ...
                    logger_1.LoggerService.error('Price feed updater error:', error);
                }
            }, this.DEX_CONFIG.priceUpdateInterval);
            logger_1.LoggerService.info('Price feed updater started');
        }
        catch (error) {
            logger_1.LoggerService.error('Start price feed updater failed:', error);
            throw error;
        }
    }
    static async startLiquidityUpdater() {
        try {
            setInterval(async () => {
                try {
                    ...
                    logger_1.LoggerService.error('Liquidity updater error:', error);
                }
            }, this.DEX_CONFIG.liquidityUpdateInterval);
            logger_1.LoggerService.info('Liquidity updater started');
        }
        catch (error) {
            logger_1.LoggerService.error('Start liquidity updater failed:', error);
            throw error;
        }
    }
    static async getQuoteFromDEX(dex, tokenIn, tokenOut, amountIn, slippage) {
        try {
            switch (dex) {
            ...
            logger_1.LoggerService.error(`Get quote from ${dex} failed:`, error);
            return null;
        }
    }
    static async getZeroXQuote(tokenIn, tokenOut, amountIn, slippage) {
        try {
            ...
            logger_1.LoggerService.error('Get 0x quote failed:', error);
            return null;
        }
    }
    static async getUniswapQuote(tokenIn, tokenOut, amountIn, slippage) {
        try {
            ...
            logger_1.LoggerService.error('Get Uniswap quote failed:', error);
            return null;
        }
    }
    static async getSushiSwapQuote(tokenIn, tokenOut, amountIn, slippage) {
        try {
            ...
            logger_1.LoggerService.error('Get SushiSwap quote failed:', error);
            return null;
        }
    }
    static async getPancakeSwapQuote(tokenIn, tokenOut, amountIn, slippage) {
        try {
            ...
            logger_1.LoggerService.error('Get PancakeSwap quote failed:', error);
            return null;
        }
    }
    static async processSwap(swap) {
        try {
            // Update swap status
            ...
            await blnkfinance_1.BlnkFinanceService.recordTransaction(`DEX swap: ${swap.tokenIn} → ${swap.tokenOut}`, [
                {
            ...
            ], swap.brokerId, swap.tokenIn, 'TRADE', swap.id, swap.metadata);
            // Update swap status
            swap.status = SwapStatus.COMPLETED;
            swap.updatedAt = new Date();
            this.swaps.set(swap.id, swap);
            logger_1.LoggerService.info('Swap processed successfully', {
                swapId: swap.id,
                status: swap.status
            });
        }
        catch (error) {
            logger_1.LoggerService.error('Process swap failed:', error);
            // Mark swap as failed
    ...
    static async processLiquidityAddition(position) {
        try {
            ...
            logger_1.LoggerService.info('Liquidity addition processed', {
                positionId: position.id,
                lpTokens: position.lpTokens,
            ...
            logger_1.LoggerService.error('Process liquidity addition failed:', error);
            throw error;
        }
    }
    static async processLiquidityRemoval(position, lpTokenAmount) {
        try {
            ...
            position.updatedAt = new Date();
            this.liquidityPositions.set(position.id, position);
            logger_1.LoggerService.info('Liquidity removal processed', {
                positionId: position.id,
                token0Return,
                token1Return,
            ...
            logger_1.LoggerService.error('Process liquidity removal failed:', error);
            throw error;
        }
    }
    static calculateSwapRiskScore(userId, amountIn, slippage) {
        try {
            let riskScore = 0;
            // Amount risk
            ...
            logger_1.LoggerService.error('Calculate swap risk score failed:', error);
            return 50; // Default medium risk
        }
    }
    static isGasOptimized(quote) {
        try {
            const gasEstimate = parseInt(quote.gasEstimate);
            return gasEstimate < 200000; // Consider optimized if under 200k gas
        }
        catch (error) {
            logger_1.LoggerService.error('Check gas optimization failed:', error);
            return false;
        }
    }
    static async updatePriceFeeds() {
        try {
            // This would typically fetch real-time prices from various sources
            logger_1.LoggerService.info('Price feeds updated');
        }
        catch (error) {
            logger_1.LoggerService.error('Update price feeds failed:', error);
        }
    }
    static async updateLiquidityPools() {
        try {
            // This would typically update pool liquidity and reserves
            logger_1.LoggerService.info('Liquidity pools updated');
        }
        catch (error) {
            logger_1.LoggerService.error('Update liquidity pools failed:', error);
        }
    }
}
