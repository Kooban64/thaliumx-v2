...
/**
 * Login endpoint - proxies to backend for email/password authentication.
 * 
 * Backend handles:
 * - Email/password validation against PostgreSQL
 * - JWT token issuance
 * - Account lockout after failed attempts
 * - MFA verification if enabled
 */
const proxyLoginRequest = async (
  apiUrls: string[], 
  body: string,
  ...
  throw lastError instanceof Error ? lastError : new Error('Login proxy request failed');
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await proxyLoginRequest(apiUrls, JSON.stringify(body), headers);
    ...
    // Forward Set-Cookie headers from backend to client (for refresh tokens)
