...
/**
 * Next.js API route to proxy /api/market/* requests to the backend
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Replace localhost with service name, or use default
    ...
    const path = request.nextUrl.pathname.replace('/api/market', '');
    const query = request.nextUrl.search;
    const apiUrl = `${backendUrl}/api/market${path}${query}`;
    ...
    console.error('Error proxying /api/market:', error);
