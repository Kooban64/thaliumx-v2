...
exports.Client = exports.defaultClient = void 0;
const caller = __importStar(require("@eeston/grpc-caller"));
...
function fullPrec(d, p) {
    return new decimal_js_1.default(d).mul(new decimal_js_1.default(10).pow(p));
}
class Client {
    client;
    markets = new Map();
    ...
    async balanceQueryByAsset(user_id, asset) {
        ...
        let available = new decimal_js_1.default(balance.available);
        let frozen = new decimal_js_1.default(balance.frozen);
    ...
    async createOrder(user_id, market, order_side, order_type, amount, price, taker_fee, maker_fee) {
        ...
        if (this.accounts.has(user_id) && (order_type == config_1.ORDER_TYPE_LIMIT || order_type == "LIMIT")) {
    ...
    async reloadMarkets(from_scratch = false) {
    ...
    createTransferTx(from, to, asset, delta, memo) {
        ...
        if (this.accounts.has(user_id)) {
            ...
            let tx = new fluidex_js_1.TransferTx({
    ...
    createWithdrawTx(account_id, asset, business, business_id, delta, detail) {
        ...
        if (this.accounts.has(account_id)) {
            let tx = new fluidex_js_1.WithdrawTx({
                account_id,
                token_id: this.assets.get(asset).inner_id,
                amount: delta,
                nonce: 0,
...
function signOrder(account, marketInfo, baseTokenInfo, quoteTokenInfo, order, checkPrec = true) {
    ...
    let amountRounded = Number(amount).toFixed(marketInfo.amount_precision);
    let priceRounded = Number(price).toFixed(marketInfo.price_precision);
    if (checkPrec && !(0, util_1.decimalEqual)(amountRounded, amount)) {
        throw new Error("invalid amount precision");
    }
    if (checkPrec && !(0, util_1.decimalEqual)(priceRounded, price)) {
    ...
    if (order_side == config_1.ORDER_SIDE_BID || order_side == "BID") {
    ...
    let orderInput = new fluidex_js_1.OrderInput({
        tokenSell,
        tokenBuy,
        totalSell,
        totalBuy,
