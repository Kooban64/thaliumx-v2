...
fn main() {
    ...
    rt.block_on(async move {
        ...
        // Optional TLS configuration (prod-v1 runs Kafka on SSL)
        if std::env::var("KAFKA_SECURITY_PROTOCOL")
            .map(|v| v.eq_ignore_ascii_case("SSL"))
            .unwrap_or(false)
        {
            kafka_cfg.set("security.protocol", "ssl");
            let ca_loc = std::env::var("KAFKA_SSL_CA_LOCATION")
                .unwrap_or_else(|_| "/etc/ssl/certs/thaliumx-ca.crt".to_string());
            kafka_cfg.set("ssl.ca.location", ca_loc);
            kafka_cfg.set("enable.ssl.certificate.verification", "true");
        }

        let consumer: StreamConsumer = kafka_cfg.create().unwrap();
