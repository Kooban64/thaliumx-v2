...
impl Settings {
    pub fn new() -> Self {
        ...
        // NOTE (Strict audit mode):
        // Keep credentials out of config files. Allow runtime override via environment variables.
        // This is critical for Docker secrets/Vault-based deployments.
        let mut settings: Settings = conf.try_into().unwrap();

        // Database URL override (used by both matchengine and restapi)
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            if !db_url.trim().is_empty() {
                settings.db_log = db_url.clone();
                settings.db_history = db_url;
            }
        }
