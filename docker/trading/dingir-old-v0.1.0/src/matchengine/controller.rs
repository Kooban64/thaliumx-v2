...
fn create_persistor(settings: &config::Settings) -> Box<dyn PersistExector> {
    // Allow disabling Kafka-backed persistence for emergency bring-up / isolation debugging.
    // Defaults remain `true` to preserve production behavior.
    let persist_to_mq = std::env::var("DINGIR_PERSIST_TO_MQ")
        .map(|v| !(v.eq_ignore_ascii_case("0") || v.eq_ignore_ascii_case("false")))
        .unwrap_or(true);
    let persist_to_mq_full_order = std::env::var("DINGIR_PERSIST_TO_MQ_FULL_ORDER")
        .map(|v| !(v.eq_ignore_ascii_case("0") || v.eq_ignore_ascii_case("false")))
        .unwrap_or(true);
