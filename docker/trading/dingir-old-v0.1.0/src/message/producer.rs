...
impl<T: MessageScheme> RdProducerContext<T> {
    pub fn new_producer(self, brokers: &str) -> Result<BaseProducer<Self>> {
        ...
        // Strict audit mode: support TLS-enabled Kafka without embedding secrets in config files.
        // Configure librdkafka SSL if requested via environment.
            ...
            .unwrap_or(false)
        {
            ...
            config.set("ssl.ca.location", &ca_loc);
